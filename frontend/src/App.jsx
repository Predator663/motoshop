...
function AppInner() {
  const { auth, systemStatus } = useApp()
  ...
  // Public pages (Privacy/Terms/About) — controlled by the superuser,
  // readable by anyone via a plain link, with no auth or setup required.
  // Placed AFTER the hooks above (never before a hook) so hook order stays
  // identical on every render regardless of the current URL.
  const legalPage = new URLSearchParams(window.location.search).get('page')
  if (['privacy', 'terms', 'about'].includes(legalPage)) {
  ...
  // Superuser bypasses maintenance mode entirely and never sees the regular
  // Shell (POS/Dashboard/etc.) — they only ever land on the control panel.
  if (auth?.role === 'superuser') return <SuperAdminPage />
  if (systemStatus?.maintenance) return <MaintenanceScreen message={systemStatus.message} />
