...
import { TIMEZONES, DEFAULT_TIMEZONE } from '../utils/datetime'
...
export default function SettingsPage() {
  ...
  const [defaultResetResult, setDefaultResetResult] = useState(null) // { username, password }
  ...
  useEffect(() => {
    ...
    setPinnedMsg(settings.cashier_pinned_message || '')
  ...
  async function updateCashierDays() {
    // Lets the owner change/extend the access window (e.g. 1 -> 2 -> 3 days)
    // WITHOUT disabling the account first. Re-uses the same enable endpoint —
    // sending is_active:true with new days always recomputes active_until
    // fresh from "now", so the old day-1 expiry is properly replaced.
    try {
      const days = accessMode === 'days' ? accessDays : null
      const result = await api.setCashierStatus(true, days)
      setCashierStatus(result.is_active)
      setCashierActiveUntil(result.active_until || null)
      toast(
        result.active_until ? `Muda umesasishwa: siku ${accessDays}` : 'Akaunti imewekwa permanent',
        'success'
      )
    } catch(err) { toast(err.message || T('settings_cashier_toggle_failed'), 'error') }
  }
  ...
  async function saveCashierUsername() {
    const u = usernameInput.trim().toLowerCase()
    if (u.length < 3) { toast('Jina la mtumiaji lazima liwe angalau herufi 3', 'error'); return }
    try {
      const result = await api.setCashierUsername(u, cashierId)
      setCashierUsername(result.username)
      setUsernameInput(result.username)
      toast('Jina la mkashia limebadilishwa', 'success')
    } catch(err) { toast(err.message || 'Imeshindwa kubadilisha jina', 'error') }
  }

  async function resetCashierToDefault() {
    // Full owner override: regardless of whatever username/password the
    // cashier last set for themselves, this issues fresh known credentials
    // and shows them once so the owner can hand them over.
    try {
      const result = await api.resetCashierDefault(cashierId)
      setCashierUsername(result.username)
      setUsernameInput(result.username)
      setDefaultResetResult(result)
      toast('Akaunti ya mkashia imerudishwa kwenye default', 'success')
    } catch(err) { toast(err.message || 'Imeshindwa kurudisha default', 'error') }
  }

  async function savePinnedMessage() {
    setPinnedSaving(true)
    try {
      await api.updateSettings({ cashier_pinned_message: pinnedMsg })
      setSettings(s => ({...s, cashier_pinned_message: pinnedMsg}))
      toast('Ujumbe umetumwa kwa mkashia', 'success')
    } catch(err) { toast(err.message || 'Imeshindwa kutuma ujumbe', 'error') }
  ...
  async function clearPinnedMessage() {
    setPinnedSaving(true)
    try {
      await api.updateSettings({ cashier_pinned_message: '' })
      setSettings(s => ({...s, cashier_pinned_message: ''}))
      setPinnedMsg('')
      toast('Ujumbe umeondolewa', 'success')
    } catch(err) { toast(err.message || 'Imeshindwa kuondoa ujumbe', 'error') }
    finally { setPinnedSaving(false) }
  }
    ...
    <div style={{maxWidth:720}}>
      ...
      <div className="card" style={{marginBottom:16}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14}}>
          ...
          <div className="input-group" style={{gridColumn:'1/-1'}}>
            <label className="input-label">Saa za Eneo (Timezone)</label>
            <select className="input" value={form.timezone||DEFAULT_TIMEZONE} onChange={set('timezone')}>
              {TIMEZONES.map(tz => <option key={tz.value} value={tz.value}>{tz.label}</option>)}
            </select>
            <span style={{fontSize:11,color:'var(--text3)',marginTop:4}}>
              Muda wote unaoonekana kwenye mfumo — risiti, ripoti, dashibodi — utaonyeshwa kwa saa hizi.
            </span>
          </div>
        </div>
      </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:12}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:'var(--text3)',marginTop:2}}>
                ...
                {cashierStatus && cashierActiveUntil && (
                  <span> · Itazimika: {formatDateTime(cashierActiveUntil)}</span>
                )}
                {cashierStatus && !cashierActiveUntil && (
                  <span> · Permanent</span>
                )}
          ...
          <div style={{padding:'4px 0 12px',borderBottom:'1px solid var(--border)'}}>
            <div style={{fontSize:12,fontWeight:600,color:'var(--text2)',marginBottom:8}}>
              {cashierStatus ? 'Badilisha Muda wa Ufikiaji' : 'Aina ya Ufikiaji Ukiwezesha'}
            </div>
            ...
            {/* Lets the owner update/extend an already-active account's
                expiry (e.g. change siku 1 -> siku 3) without needing to
                Zima first — that old flow silently kept the day-1 expiry. */}
            {cashierStatus && (
              <button
                className="btn btn-sm btn-primary"
                style={{marginTop:10}}
                onClick={updateCashierDays}
              >Sasisha Muda</button>
            )}
          </div>
          ...
          <div className="flex items-center justify-between" style={{padding:'12px 0',borderBottom:'1px solid var(--border)'}}>
            <div style={{flex:1}}>
              <div style={{fontWeight:600}}>Jina la Mtumiaji (Username)</div>
              <div style={{fontSize:12,color:'var(--text3)',marginTop:2,marginBottom:8}}>
                Ukibadilisha, mkashia atatakiwa kutumia hili wakati ujao anapoingia.
              </div>
              <div className="flex gap-2" style={{flexWrap:'wrap'}}>
                <input
                  ...
                  placeholder="jina.la.mkashia"
                />
                <button
                  className="btn btn-primary btn-sm"
                  onClick={saveCashierUsername}
                  disabled={usernameInput.trim().toLowerCase() === cashierUsername}
                >Sasisha Jina</button>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between" style={{padding:'12px 0'}}>
            <div>
              <div style={{fontWeight:600}}>Rudisha kwa Default (Full Reset)</div>
              <div style={{fontSize:12,color:'var(--text3)',marginTop:2}}>
                Bila kujali mkashia amebadilisha nini (jina au nywila), hii inaunda jina na
                nywila mpya kabisa — unazipata papo hapo kukabidhi kwa mkashia.
              </div>
            </div>
            <button className="btn btn-danger btn-sm" onClick={resetCashierToDefault}>Rudisha Default</button>
          </div>
        </div>
      </div>

      {/* Pinned message to cashier */}
      <div className="card" style={{marginBottom:16}}>
        <div className="section-title">Ujumbe wa Kudumu kwa Mkashia (Pinned)</div>
        <div style={{fontSize:12,color:'var(--text3)',marginBottom:10}}>
          Ujumbe huu utaonekana juu kwenye mfumo wa mkashia mpaka utakapoufuta hapa —
          mkashia hawezi kuuondoa mwenyewe.
        </div>
        <textarea
          className="input"
          rows={3}
          style={{resize:'vertical',marginBottom:10}}
          placeholder="Mfano: Kumbuka kufunga zamu kila unapomaliza kazi..."
          value={pinnedMsg}
          onChange={e => setPinnedMsg(e.target.value)}
        />
        <div className="flex gap-2" style={{flexWrap:'wrap'}}>
          <button className="btn btn-primary btn-sm" onClick={savePinnedMessage} disabled={pinnedSaving}>
            {pinnedSaving ? <span className="spinner" /> : (settings.cashier_pinned_message ? 'Sasisha Ujumbe' : 'Tuma Ujumbe')}
          </button>
          {!!settings.cashier_pinned_message && (
            <button className="btn btn-secondary btn-sm" onClick={clearPinnedMessage} disabled={pinnedSaving}>Ondoa Ujumbe</button>
          )}
        </div>
      </div>
      ...
      {defaultResetResult && (
        <Modal title="Akaunti Mpya ya Mkashia" onClose={() => setDefaultResetResult(null)}
          footer={<button className="btn btn-primary" onClick={() => setDefaultResetResult(null)}>Nimeshaandika, Funga</button>}
        >
          <div style={{fontSize:13,color:'var(--text2)',marginBottom:12}}>
            Andika/tuma haya kwa mkashia sasa — hayataonyeshwa tena baada ya kufunga dirisha hili.
          </div>
          <div className="input-group">
            <label className="input-label">Jina la Mtumiaji</label>
            <input className="input" readOnly value={defaultResetResult.username} onFocus={e => e.target.select()} />
          </div>
          <div className="input-group">
            <label className="input-label">Nywila</label>
            <input className="input" readOnly value={defaultResetResult.password} onFocus={e => e.target.select()} />
          </div>
        </Modal>
      )}
