...
export default function ReportsPage() {
  ...
  // FIX (pro PDF exports): downloads the real server-generated PDF (shop
  // letterhead, logo, branded tables) for whichever report tab is active —
  // no longer relies on the browser's window.print() dialog.
  const downloadPdf = useCallback(async () => {
    setPdfLoading(true)
    try {
      if (tab === 'pl') await api.downloadReportPLPdf(from, to)
      else if (tab === 'stock') await api.downloadReportStockPdf()
      else if (tab === 'sales') await api.downloadReportSalesPdf(from, to)
      else if (tab === 'aging') await api.downloadReportDebtAgingPdf()
      toast('PDF imepakuliwa', 'success')
    } catch(e) { console.error(e); toast(e.message || 'Imeshindwa kupakua PDF', 'error') }
    ...
    <div>
      <div className="page-header">
        ...
        <button className="btn btn-secondary btn-sm" onClick={downloadPdf} disabled={pdfLoading}>
          {pdfLoading ? <span className="spinner" /> : '⬇️ Download PDF'}
        </button>
      </div>
