...
async function request(method, path, body, signal) {
  ...
  if (!res.ok) throw { status: res.status, message: data.error || 'Request failed', maintenance: !!data.maintenance }
  return data
}
...
// FIX (pro PDF exports): reports are now generated server-side as real,
// letterhead-branded PDFs (see /api/reports/*/pdf) instead of relying on
// window.print(). This fetches the PDF as a blob (auth header attached,
// same as every other request) and triggers a native file download.
async function downloadFile(path, filename) {
  ...
  if (!res.ok) {
    const data = await res.json().catch(() => ({}))
    throw { status: res.status, message: data.error || 'Imeshindwa kupakua PDF' }
  }
...
async function uploadFile(path, file, extraFields) {
  ...
  if (extraFields) for (const k in extraFields) form.append(k, extraFields[k])
  let res
  try {
...
// FIX (cashier offline): the SW/queue system already lets an *already
// logged-in* cashier keep working with zero network (products, shifts,
// sales all cache/queue). The one real gap was the very first thing that
// happens each session — login — which always needed the server to check
// the password. If the shop has no network at all when the cashier opens
// the app (fresh device reboot, browser data not cleared but truly
// offline), they could never get in.
//
// Fix: on every successful ONLINE login, cache a SHA-256 hash of the
// password (never the password itself) alongside the session token for
// that device. If a later login attempt fails because there's genuinely no
// network (not a wrong-password rejection — that still comes from the
// server when reachable), and the typed username+password hash matches
// what's cached, we resume the last known session token locally. This is
// the same trust model as any offline-capable mobile app: the device
// already held a valid session; going offline doesn't revoke it.
const OFFLINE_CREDS_KEY = 'motoshop_offline_creds'

async function sha256Hex(text) {
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(text))
  return Array.from(new Uint8Array(buf)).map(b => b.toString(16).padStart(2, '0')).join('')
}
function getOfflineCreds() {
  try { return JSON.parse(localStorage.getItem(OFFLINE_CREDS_KEY) || '{}') } catch { return {} }
}
function cacheOfflineCred(username, passwordHash, data) {
  try {
    const creds = getOfflineCreds()
    creds[username] = { hash: passwordHash, token: data.token, role: data.role, username: data.username, user_id: data.user_id }
    localStorage.setItem(OFFLINE_CREDS_KEY, JSON.stringify(creds))
  } catch {}
}
...
export const api = {
  ...
  login:              async (u, p) => {
    ...
    try {
      ...
      try { cacheOfflineCred(uname, await sha256Hex(p), data) } catch {}
      return data
    } catch (err) {
      if (!isOffline(err)) throw err
      const rec = getOfflineCreds()[uname]
      if (rec) {
        try {
          const hash = await sha256Hex(p)
          if (hash === rec.hash) return { token: rec.token, role: rec.role, username: rec.username, user_id: rec.user_id, offline: true }
        } catch {}
      }
      throw { status: undefined, networkError: true,
        message: rec
          ? 'Nywila hailingani na kifaa hiki. Jaribu tena au unganisha mtandao.'
          : 'Hakuna mtandao na kifaa hiki hakijawahi kuingia na akaunti hii kabla — unganisha mtandao kwa mara moja tu ya kwanza.' }
    }
  },
  ...
  setCashierUsername: (username, cashier_id) => api.put('/auth/cashier-username', { username, ...(cashier_id != null ? { cashier_id } : {}) }),
  resetCashierDefault:(cashier_id) => api.post('/auth/cashier-reset-default', { ...(cashier_id != null ? { cashier_id } : {}) }),

  // Public — used before login too, to show the maintenance screen
  getSystemStatus: () => api.get('/system-status'),
  getLegalContent: () => api.get('/legal-content'),

  // Superuser control panel
  adminGetLegalContent:   () => api.get('/admin/legal-content'),
  adminUpdateLegalContent:(data) => api.put('/admin/legal-content', data),

  // Superuser control panel
  adminListTables:   () => api.get('/admin/tables'),
  adminGetRows:       (table, page = 1, limit = 50, q = '') => api.get(`/admin/tables/${encodeURIComponent(table)}?page=${page}&limit=${limit}&q=${encodeURIComponent(q)}`),
  adminGetRow:        (table, id) => api.get(`/admin/tables/${encodeURIComponent(table)}/${id}`),
  adminCreateRow:     (table, data) => api.post(`/admin/tables/${encodeURIComponent(table)}`, data),
  adminUpdateRow:     (table, id, data) => api.put(`/admin/tables/${encodeURIComponent(table)}/${id}`, data),
  adminDeleteRow:     (table, id) => api.delete(`/admin/tables/${encodeURIComponent(table)}/${id}`),
  adminGetSystem:     () => api.get('/admin/system'),
  adminUpdateSystem:  (data) => api.put('/admin/system', data),
  adminListBackups:   () => api.get('/admin/backups'),
  adminCreateBackup:  () => api.post('/admin/backups', {}),
  adminDeleteBackup:  (filename) => api.delete(`/admin/backups/${encodeURIComponent(filename)}`),
  adminDownloadBackup:(filename) => downloadFile(`/admin/backups/${encodeURIComponent(filename)}`, filename),
  adminRestoreBackup: (filename) => api.post(`/admin/backups/${encodeURIComponent(filename)}/restore`, {}),
  adminUploadBackup:  (file) => uploadFile('/admin/backups/upload', file),
  ...
  // Professional PDF exports (server-generated, letterhead branded)
  downloadReportPLPdf:        (from, to) => downloadFile(`/reports/pl/pdf?from=${from}&to=${to}`, `PL-Report_${from}_to_${to}.pdf`),
  downloadReportStockPdf:     () => downloadFile('/reports/stock-valuation/pdf', `Stock-Valuation_${today()}.pdf`),
  downloadReportDebtAgingPdf: () => downloadFile('/reports/debt-aging/pdf', `Debt-Aging_${today()}.pdf`),
  downloadReportSalesPdf:     (from, to) => downloadFile(`/reports/sales-statement/pdf?from=${from}&to=${to}`, `Sales-Statement_${from}_to_${to}.pdf`),
}
...
// Date/time formatting and "today"/"start of month" all live in
// utils/datetime.js now — they're timezone-aware (shop Settings ->
// timezone, defaulting to EAT) instead of using the device's own clock.
// Re-exported here so existing `from '../utils/api'` imports don't need
// to change across every page.
export { formatDate, formatDateTime, formatTime, formatDateLong, todayInTZ as today, monthStartInTZ as monthStart } from './datetime'
