...
self.addEventListener('fetch', evt => {
  ...
  if (req.mode === 'navigate') {
    evt.respondWith(
      fetch(req).then(resp => {
        // FIX: only cache successful shells. Caching a transient error
        // response (e.g. a 404/502 seen mid-deploy) here would otherwise
        // get served back to the user on every future visit, even long
